...
(function(ext) {
  ...
  function init() {
    ...
    setTimeout(queryCapabilities(), 1000);
  ...
  function processSysexMessage() {
    sysexResponse = storedInputData[0];
    console.log('Received sysex response '+sysexResponse.toString(16));
    switch(sysexResponse) {
      case CAPABILITY_RESPONSE:
        console.log('Received capability response.');
        for (var i = 1, pin = 0; pin < MAX_PINS; pin++) {
          while (storedInputData[i++] != 0x7F) {
            var storedData = storedInputData[i-1];
            // This helped reduce error over a noisy wireless serial connection
            if ( storedData <= MAX_PINS )
            {
              pinMode = pinModes[storedData];
      ...
      case ANALOG_MAPPING_RESPONSE:
        console.log('Received analog mapping response.');
        ...
        break;
      case QUERY_FIRMWARE:
        console.log('Received firmware response.');
        if (!connected) {
          console.log('Connection verified.');
          // Initial response from board. Set things up now.
          clearInterval(pingUntilReady);
          pingUntilReady = null;
...
//        console.log('Additional sysex data '+inputData[i].toString(16));
        ...
        switch(command) {
          ...
          case ANALOG_MESSAGE:
          case REPORT_VERSION:
            console.log('Began parsing message '+command.toString(16));
            waitForData = 2;
  ...
  function tryNextDevice() {
    ...
    pingUntilReady = setInterval(function() {
      console.log('Pinging firmware for initial connection verification.');
      queryFirmware();
    }, 1000);
  ...
  ext._shutdown = function() {
    ...
    if (device) device.close();
    if (pingUntilReady) clearInterval(pingUntilReady);
    device = null;
  };
